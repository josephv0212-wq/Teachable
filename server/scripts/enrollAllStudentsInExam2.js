...
async function enrollAllStudentsInExam2() {
  try {
    ...
    if (!exam2Course) {
      ...
      exam2Course = await get('SELECT id FROM courses WHERE id = ?', [courseResult.id]);
      console.log('Exam2 course created with ID:', exam2Course.id);
    } else {
      console.log('Exam2 course found with ID:', exam2Course.id);
    }

    // Get all students
    const students = await all('SELECT id FROM users');
    console.log(`Found ${students.length} students`);

    if (students.length === 0) {
      console.log('No students found. Nothing to enroll.');
      process.exit(0);
    }

    // Enroll each student in exam2
    let enrolledCount = 0;
    let skippedCount = 0;
    let errorCount = 0;
    ...
    for (const student of students) {
      try {
        ...
        const existingEnrollment = await get(
          ...
          [student.id, exam2Course.id]
        );

        if (existingEnrollment) {
          console.log(`Student ${student.id} already enrolled in exam2. Skipping.`);
          skippedCount++;
          continue;
        }
        ...
        await run(
          ...
          [student.id, exam2Course.id, 'paid']
        );

        console.log(`Enrolled student ${student.id} in exam2`);
        enrolledCount++;
      } catch (error) {
        console.error(`Error enrolling student ${student.id}:`, error.message);
        errorCount++;
      }
    }
    ...
    console.error('Error enrolling students in exam2:', error);
    process.exit(1);
  }
}

enrollAllStudentsInExam2();
